...
from schemas.office_schema import OfficeCreate, OfficeGet, OfficeUpdatePhoneNumber, OfficeGetWithServices
...
class OfficeService:
    ...
    def get_offices(
        ...
        *,
        ...
        limit: int = 100,
        offset: int = 0,
    ) -> list[OfficeGetWithServices]:
        return [
            OfficeGetWithServices.model_validate(office)
            for office in office_repository.get_all(order=order, limit=limit, offset=offset)
        ]

    def get_office_by_id(self, pk: int) -> OfficeGetWithServices:
        return OfficeGetWithServices.model_validate(office_repository.get_single(id=pk))

    def get_office_by_address(self, address: str) -> OfficeGetWithServices:
        return OfficeGetWithServices.model_validate(office_repository.get_single(address=address))

    def change_office_phone_number(self, pk: int, phone_number: str) -> OfficeGet:
        return OfficeGetWithServices.model_validate(
